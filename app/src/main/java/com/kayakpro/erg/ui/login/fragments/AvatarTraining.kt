...
import com.kayakpro.erg.SpriteView
...
class AvatarTraining : Fragment() {
    ...
    private var hasAnchoredBaseline = false
    ...
    override fun onDestroyView() {
        binding.ivAvatarSelf.stopSpriteAnimation()
        binding.ivAvatarComp.stopSpriteAnimation()
...
//                binding.gifImageView.setImageDrawable(GifDrawable(inputStream))
    ...
    private fun stopAnimation(b: Boolean) {
        if (!b) {
            // changed by tona =========================================================================================
            val deviceType = sp.getSelectedMachine()
            if(deviceType == "Dragon Boat") {
                binding.ivAvatarSelf.setupSprite("sprite_1_0.png", 70L, 9)
                binding.ivAvatarComp.setupSprite("sprite_1_1.png", 65L, 9)
            } else {
                binding.ivAvatarSelf.setupSprite("sprite_0_0.png", 70L, 11)
                binding.ivAvatarComp.setupSprite("sprite_0_1.png", 65L, 11)
            }
            binding.ivAvatarSelf.startSpriteAnimation()
            binding.ivAvatarComp.startSpriteAnimation()
            // end by tona =============================================================================================
        } else {
            binding.ivAvatarSelf.stopSpriteAnimation()
            binding.ivAvatarComp.stopSpriteAnimation()
    ...
    private fun freezeGif(imageView: ImageView) {
        if (imageView is SpriteView && imageView.isSpriteAnimating()) {
            imageView.stopSpriteAnimation()
            val frameBitmap = imageView.getCurrentFrameBitmap()
            if (frameBitmap != null) {
    ...
    private fun updateRowedDistanceFromConsole() {
        ...
        if (current != lastConsoleDistance) {
            if (!hasAnchoredBaseline && lastConsoleDistance != -1) {
                firstDistance = current
                hasAnchoredBaseline = true
            }
            lastConsoleDistance = current
        }
        // changed by tona ==================================================================================
        // myDistance = if (!hasAnchoredBaseline) 0 else kotlin.math.max(0, firstDistance - current)
        myDistance = if (!hasAnchoredBaseline) 0 else kotlin.math.max(0, kotlin.math.abs(firstDistance - current) + 1 )
        // end by tona ======================================================================================
        val isPlayer2Mode = arguments?.getBoolean("isPlayer2Mode", false) ?: false
    ...
    private fun updateDistanceGoalFromConsole() {
        ...
        if (hasAnchoredBaseline) {
            // changed by tona ======================================================================
            // deadline = listDistance - kotlin.math.max(0, firstDistance - current)
            deadline = listDistance - kotlin.math.max(0, kotlin.math.abs(firstDistance - current) + 1 )
            // end by tona ==========================================================================
                ...
                override fun run() {
                    ...
                    handler.post {
                        ...
                        if (isRunning) {
                            ...
                            caloriesServer = Math.round(consoleCal / 3600.0).toString()
    ...
    fun saveData(){
        ...
        alWorkoutData.add(
            CreateTrainingHistoryRequest.Workout(
                distanceServer,
                speedServer,
                (tiktik * 1000).toLong().toString(),
                caloriesServer,
                wattsServer,
                paceServer,
                strokeRateServer,
                (timemilli * 1000).toLong().toString(),
                heartRateServer
            )
        )
    ...
    private fun submitHistory(trainingId: String, alWorkoutData: ArrayList<CreateTrainingHistoryRequest.Workout>) {
        Log.d("AvatarTraining", "submitHistory: type=Avatar, trainingId=$trainingId, isTimeOrDistance=$isTimeOrDistanceServer, value=$valueServer, paceForBoat=$paceForBoatServer")

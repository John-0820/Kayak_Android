...
class QuickStartTraining : Fragment(), OnClickQuickItem {
            ...
            timer.schedule(object : TimerTask() {
                override fun run() {
                    ...
                    handler.post {
                        ...
                        alValues.forEach { model ->
                            ...
                            if (model.name.contains("istance")) {
                                if(!isTime)
                                    ...
                                    model.value = (if (!hasAnchoredBaseline) 0 else kotlin.math.abs(firstDistance - BleRepository.distance.value.toInt())).toString()
                            }
                        ...
                        calorieServer = Math.round(consoleCal / 3600.0).toString()
                        wattsServer = watts
    ...
    private fun sendData() {
        // The backend's `type` enum has no "QuickStart" value — it only
        // recognizes "Program"/"Avatar" (confirmed: server returns
        // HTTP 500 "Invalid Training Type" for type="QuickStart", but
        // 200 for type="Avatar"). Quick Start has no backing training
        // entity, so we submit it as "Avatar" (accepted by the server)
        // and pass the real "QuickStart" label separately so the
        // History page still displays it correctly via HistoryTypeCache.
        ...
        Log.d("QuickStartTraining", "sendData: type=${createTrainingHistoryRequest.type}")
        viewModel.createTrainingHistory(
            createTrainingHistoryRequest,
            "Bearer " + sp.getAToken().toString(),
            displayTypeOverride = "QuickStart"
        )
    }
